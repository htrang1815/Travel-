...
const UpdateBlogPage = () => {
  ...
  const updateBlog = async () => {
    ...
    console.log("hello");
    const res = await axios.patch(`${domain}/api/v1/blogs/${blogId}`, {
      title: title || blog.title,
      article: article || blog.article,
      user: user._id,
      bannerImage: imageCover || blog.bannerImage,
    });
    console.log(res);
    // const blogId = res.data.data.blogs;
    ...
    <div className="bg-[#111] ">
      ...
      <div className="bg-[#111] w-full pt-[78px] h-[400px] px-[5%] ">
        <div className=" w-full h-full text-primary  bg-cover bg-center relative ">
          <img
            src={imageCover || blog?.bannerImage || imgdefault}
            alt=""
